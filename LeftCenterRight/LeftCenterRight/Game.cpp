	...
	:numPlayers(0)
{
	...
	while (numPlayers < 3)
	{
		//Prompt user for number of players and assign to numPlayers
		Console::Write("How many players? ");

		//Try to get and assign user input to number of players
		try
		{
			numPlayers = Convert::ToInt32(Console::ReadLine());
		}

		//Catch the 2 exceptions that can be thrown by  Convert::ToInt32. 
		catch (Exception^ e)
		{
			//User didn't enter an integer
			if (dynamic_cast<FormatException^>(e))
			{
				Console::WriteLine("Input is invalid, it must be an integer.");
				numPlayers = 0;
			}

			//Intger entered is too big
			else if (dynamic_cast<OverflowException^>(e))
			{
				Console::WriteLine("That is WAY WAY WAY too many players.");
				numPlayers = 0;
			}
		}
		...
		if (numPlayers < 3)
		{
			Console::WriteLine("There must be at least 3 players.");
		}
	}
	...
	for (int i = 0; i < numPlayers; i++)
	{
		...
		playerArray[i] = gcnew Player(Console::ReadLine()); 
